	...
	const processedContent = await Promise.all(
		content.split('\n').map(async (line) => {
			...
			try {
				...
				const res = `<div class="admonition github max-w-2xl mx-auto my-8 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800"><div class="repo-details p-6 flex items-start space-x-6"><img src="${repoData.owner.avatar_url}" alt="${owner}" class="h-24 w-24 rounded-full bg-gray-100 ring-2 ring-gray-300/50 dark:bg-gray-700 dark:ring-gray-600/50"/><div class="flex-1"><h3 class="text-lg font-semibold text-gray-900 dark:text-white">${repoData.full_name}</h3><div class="repo-stats mt-2 flex items-center space-x-4 text-sm text-gray-600 dark:text-gray-400"><span class="stat inline-flex items-center gap-1"><svg class="h-4 w-4" viewBox="0 0 16 16" fill="currentColor"><path d="M8 .25a.75.75 0 0 1 .673.418l1.882 3.815 4.21.612a.75.75 0 0 1 .416 1.279l-3.046 2.97.719 4.192a.75.75 0 0 1-1.088.791L8 12.347l-3.766 1.98a.75.75 0 0 1-1.088-.79l.72-4.194L.818 6.374a.75.75 0 0 1 .416-1.28l4.21-.611L7.327.668A.75.75 0 0 1 8 .25z"/></svg>${repoData.stargazers_count.toLocaleString()}</span><span class="stat inline-flex items-center gap-1"><svg class="h-4 w-4" viewBox="0 0 16 16" fill="currentColor"><path d="M5 5.372v.878c0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75v-.878a2.25 2.25 0 1 1 1.5 0v.878a2.25 2.25 0 0 1-2.25 2.25h-1.5v2.128a2.251 2.251 0 1 1-1.5 0V8.5h-1.5A2.25 2.25 0 0 1 3.5 6.25v-.878a2.25 2.25 0 1 1 1.5 0ZM5 3.25a.75.75 0 1 0-1.5 0 .75.75 0 0 0 1.5 0Zm6.75.75a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5Zm-3 8.75a.75.75 0 1 0-1.5 0 .75.75 0 0 0 1.5 0Z"/></svg>${repoData.forks_count.toLocaleString()}</span></div>${repoData.description ? `<p class="mt-2 text-sm text-gray-600 dark:text-gray-400">${repoData.description}</p>` : ''}</div></div>${repoData.languages.length > 0 ? `<div class="languages border-t border-gray-100 bg-gray-50 px-6 py-4 dark:border-gray-700 dark:bg-gray-800/50"><div class="flex flex-wrap gap-2 text-sm">${repoData.languages.map((lang) => `<span class="tag ${lang.toLowerCase()}">${normalizedTechnologyName(lang)}</span>`).join('')}</div></div>` : ''}</div>`;
				return res;
			} catch (error) {
