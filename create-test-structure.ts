import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

const structure = {
  test: {
    config: ["base.config.test.ts"],
    controllers: [
      "articles.controller.test.ts",
      "auth.controller.test.ts",
      "docs.controller.test.ts",
      "resume.controller.test.ts",
      "series.controller.test.ts",
      "tags.controller.test.ts",
      "transaction.controller.test.ts",
    ],
    middlewares: ["auth.middleware.test.ts", "logger.middleware.test.ts"],
    models: [
      "docs.model.test.ts",
      "article.model.test.ts",
      "resume.model.test.ts",
      "transaction.model.test.ts",
      "user.model.test.ts",
    ],
    routes: [
      "auth.routes.test.ts",
      "docs.routes.test.ts",
      "transaction.routes.test.ts",
      "resume.routes.test.ts",
      "series.routes.test.ts",
      "tags.routes.test.ts",
      "articles.routes.test.ts",
    ],
    services: [
      "auth.service.test.ts",
      "db.service.test.ts",
      "docs.services.test.ts",
      "transaction.service.test.ts",
      "resume.service.test.ts",
      "series.service.test.ts",
      "tags.service.test.ts",
      "article.service.test.ts",
    ],
    utils: {
      files: [
        "docs.utils.test.ts",
        "error.utils.test.ts",
        "file.utils.test.ts",
        "article.utils.test.ts",
        "certs.utils.test.ts",
      ],
      parsers: [
        "csv.parsers.test.ts",
        "factory.parsers.test.ts",
        "pdf.parsers.test.ts",
      ],
    },
    websockets: ["transaction.websocket.test.ts"],
  },
};

function createStructure(basePath: string, structure: Record<string, any>) {
  for (const key in structure) {
    const currentPath = path.join(basePath, key);

    if (Array.isArray(structure[key])) {
      // Create the folder if it doesn't exist
      if (!fs.existsSync(currentPath)) {
        fs.mkdirSync(currentPath, { recursive: true });
      }
      structure[key].forEach((file: string) => {
        const filePath = path.join(currentPath, file);
        if (!fs.existsSync(filePath)) {
          // Write a simple Vitest test that always passes
          const testContent = `// Auto-generated Vitest file: ${file}

import { describe, it, expect } from 'vitest';

describe('${file}', () => {
  it('should always pass', () => {
    expect(true).toBe(true);
  });
});
`;
          fs.writeFileSync(filePath, testContent, "utf-8");
        }
      });
    } else if (typeof structure[key] === "object") {
      // Create the folder if it doesn't exist and then process its structure recursively
      if (!fs.existsSync(currentPath)) {
        fs.mkdirSync(currentPath, { recursive: true });
      }
      createStructure(currentPath, structure[key]);
    }
  }
}

// Run the script
const basePath = path.resolve(__dirname, "test");
if (!fs.existsSync(basePath)) {
  fs.mkdirSync(basePath, { recursive: true });
}
createStructure(basePath, structure.test);

console.log("Vitest test structure created successfully!");
