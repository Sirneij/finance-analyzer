...
async def analyze_transactions(transactions: list[dict], ws_manager: WebSocketManager = None) -> dict:
    try:
        ...
        if ws_manager:
            await ws_manager.send_progress('Detecting anomalies...', 0.65, 'Analysis')
        anomalies = await asyncio.to_thread(_detect_anomalies, tx_objects)

        if ws_manager:
            await ws_manager.send_progress('Analyzing spending...', 0.72, 'Analysis')
        spending_analysis = await asyncio.to_thread(_analyze_spending, tx_objects)

        if ws_manager:
            await ws_manager.send_progress('Predicting trends...', 0.80, 'Analysis')
        ...
        recurring_transactions = await asyncio.to_thread(_analyze_recurring_transactions, tx_objects)

        if ws_manager:
            await ws_manager.send_progress('Calculating financial health...', 0.96, 'Analysis')
        financial_health = await asyncio.to_thread(_calculate_financial_health, tx_objects)
...
async def classify_transactions(transactions: list[Transaction], ws_manager: WebSocketManager = None) -> dict:
    ...
    # Combine common patterns with environment labels
    ...
    for batch_idx in range(total_batches):
        ...
        # Use ML only for unmatched transactions
        try:
...
def _analyze_recurring_transactions(transactions: list[Transaction]) -> dict:
...
def _calculate_financial_health(transactions: list[Transaction]) -> dict:
    ...
    return {
        ...
        'financial_health_score': _calculate_health_score(debt_to_income, savings_rate, balance_growth),
    }


def _calculate_health_score(debt_ratio: float, savings_rate: float, balance_growth: float) -> int:
